...
SELECT TO_DATE('99/10/11','YY/MM/DD') FROM DUAL; -- 2099-10-11 00:00:00.0|00
...
-- R 타입은 1950 이전은 2000년대로 해석함

-- 8. 춘 기술대학교의 2000 년도 이후 입학자들은 학번이 A 로 시작하게 되어있다. 2000 년도 이전 학번을 받은
...
-- 11. 지도 교수를 배정받지 못한 학생 수는 몇 명 정도 되는 지를 알아내는 SQL 문을 작성하시오
...
-- 단, 이때 출력 화면의 헤더는 "년도", "년도 별 평점" 이라고 찍히게 하고 점수는 반올림하여 소수점 이하 한자리 까지만 표시
SELECT * FROM TB_GRADE; 
...
-- 15. 학번이 A112113 인 김고운 학생의 년도, 학기 별 평점과 년도 별 누적평점, 총평점을 구하는
-- SQL 문을 작성하시오 (단, 평점은 소수점 1자리까지만 반올리하여 표시한다)
