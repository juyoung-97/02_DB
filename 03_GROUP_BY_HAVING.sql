...
-- EMPLOYEE 테이블에서 (FROM)
-- 직급코드가 같은 사람의 (GROUP BY)
-- 직급코드, 급여 평균, 인원수를 (SELECT)
...
-- EMPLOYEE 테이블에서 (FROM)
-- 성별(남/여)과(GROUP BY) 각 성별 별 인원수, 급여 합을 (SELECT)
...
ORDER BY "인원 수"; -- 별칭 사용 O (SELECT 해석 완료된 후)

-----------------------------------------------------------------
-- *WHERE 절 GROUP BY 절 혼합하여 사용하기 *

--> WHERE 절은 각 컬럼값에 대한 조건
--> HAVING 절은 그룹에 대한 조건 ******매우 중요******

-- EMPLOYEE 테이블에서
-- 부서코드가 D5, D6인 부서의 
-- 부서코드, 평균급여, 인원 수 조회
SELECT DEPT_CODE, ROUND(AVG(SALARY)) 평균, COUNT(*) "인원 수"
FROM EMPLOYEE
...
-- EMPLOYEE 테이블에서 (FROM)
-- 부서별로 같은직급인 사원의 인원수를 조회 (GROUP BY)
...
-- 집계 함수(ROLLUP, CUBE)
-- 그룹 별 산출 겨로가값의 집계를 계산하는 함수
-- (그룹별로 중간 집계 결과를 추가)
-- GROUP BY 절에서만 사용할 수 있는 함수! 

-- ROLLUP : GROUP BY 절에서 가장먼저 작성된 컬럼의 중간집계를 처리하는 함수
...
-- UNION ALL
-- MINUS 
-- 급여가 300만 초과인 사원의 사법 이름 부서코드 급여 조회
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
