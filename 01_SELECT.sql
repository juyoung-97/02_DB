...
SELECT '같음' 
FROM DUAL -- DUAL(DUmmy tAbLe) : 가짜 테이블(임시 조회용 테이블)
WHERE 1 = '1';
...
-- '_' 예시
-- 'A_' : A로 시작하는 두 글자 문자열
-- '____A' : A로 끝나는 5 글자 문자열
-- '__A__' : 세번쨰 문자가 A인 5 글자 문자열
-- '_____' : 다섯글자 문자열
...
FROM EMPLOYEE
WHERE EMAIL LIKE '____%'; -- 잘못된방법 ESCAPE 사용해야함
