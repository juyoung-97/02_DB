...
-- LOCATION 테이블에서 NATIONAL_CODE 가 KO 인 경우의 LOCAL_CODE와
-- DEPARTMENT 테이블의 LOCATION_ID 와 동일한 DEPT_ID가
-- EMPLOYEE 테이블의 DEPT_CODE 와 동일한 사원을 구하시오
...
WHERE LOCATION_ID = (SELECT LOCAL_CODE 
FROM LOCATION
WHERE NATIONAL_CODE = 'KO')); -- 다중행일경우엔 =연산자 대신 IN 사용
...
/*3*/SELECT ROWNUM, EMP_NAME, SALARY
/*1*/FROM (SELECT EMP_NAME, SALARY
			FROM EMPLOYEE
			ORDER BY SALARY DESC) -- 해석순위 1위 FROM 절에서 전체직원의 SALARY 내림차순 정렬
/*2*/WHERE ROWNUM <= 5; -- 해석순위 2위 WHERE 절에서 이제 가상컬럼의 1~5 행까지만 조회
