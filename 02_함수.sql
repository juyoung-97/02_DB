...
-- 함수는 SELECT 문의
-- SELECT 절, WHERE절,  ORDER BY절,  GROUP BY절,  HAVING 절에서 사용 가능
...
-- EMPLOYEE 테이블에서 사원의 월급을 100만으로 나눴을때 나머지 값 조회
...
SELECT TRUNC(123.456) FROM DUAL; -- 소수점 아래를 절삭
SELECT TRUNC(123.456, 1) FROM DUAL; -- 소수점 첫째자리 아래를 절삭

------------------------------------------------------------

-- 날짜(DATE) 관련 함수

-- SYSDATE : 시스템 상의 현재 시간 (년,월,일,시,분,초) 반환
SELECT SYSDATE FROM DUAL; -- 2025-10-20 09:14:55.000

-- SYSTIMESTAMP : SYSDATE + MS 단위 추가(UTC 정보)
SELECT SYSTIMESTAMP FROM DUAL; -- 2025-10-20 09:15:58.132 +0900

-- MONTH_BETWEEN(날짜, 날짜) : 두 날짜의 개월 수 차이 반환
SELECT ABS(ROUND(MONTHS_BETWEEN(SYSDATE, '2026-02-27'), 1)) "수강 기간(개월)" FROM DUAL;
---4.2
...
SELECT ADD_MONTHS(SYSDATE, 4) FROM DUAL; -- 2026-02-20 09:30:11.000
SELECT ADD_MONTHS(SYSDATE, -1) FROM DUAL; -- 2025-09-20 09:31:00.000

-- LAST_DAY(날짜) : 해당 달의 마지막 날짜를 구함
SELECT LAST_DAY(SYSDATE) FROM DUAL; -- 2025-10-31 09:32:10.000
...
--> 패턴을 적용해서 작성된 문자열의 각 문자가 어떤 날자 형식인지 인식 시킴
...
-- NVL(컬럼명, 컬럼값이 NULL일 때 바꿀값) : NULL 인 컬럼값을 다른값으로 변경
...
-- NVL2(컬럼명, 바꿀값1, 바꿀값2)
-- 해당 컬럼의 값이 있으면 바꿀값1 변경, NULL 이면 바꿀값2 변경
...
-- 급여 최소값, 가장빠른 입사일, 알파벳 순서가 가장빠른 이메일 조회
SELECT MIN(SALARY), MIN(HIRE_DATE), MIN(EMAIL)
FROM EMPLOYEE;

-- 급여 최대값, 가장늦은 입사일, 알파벳 순서가 가장 뒤에있는 이메일 조회
