...
ON(DEPT_CODE = DEPT_ID); -- 23행 (NULL인 2명 포함)
...
-- '=' (등호)를 사용하지 않는 JOIN 문
-- 지정된 컬럼값이 일치하는 경우가 아닌, 값의 범위에 포함되는 행들을 연결하는 방식
...
-- 테이블마다 별칭 작성(미작성 시 열의정의 애매하다.. 오류발생)
...
SELECT EMP_NAME, JOB_NAME, SALARY, NVL((SALARY + (SALARY * BONUS)) * 12, SALARY  * 12) 연봉
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
...
-- 주의사항!
-- NATURAL JOIN 을 다중조인에서 사용할 때는
-- SELECT 절에 NATURAL JOIN 에서
-- 사용되었을 연결고리 컬럼을 반드시 작성해야함

SELECT EMP_NAME, JOB_NAME, SALARY, JOB_CODE, NVL((SALARY + (SALARY * BONUS)) * 12, SALARY  * 12) 연봉
FROM EMPLOYEE
NATURAL JOIN JOB -- JOB_CODE 사용했으니 SELECT절에 이내용이 포함되어야함
JOIN SAL_GRADE USING(SAL_LEVEL)
WHERE SALARY > MIN_SAL;
...
--9. 보너스포인트가 없는 직원들 중에서 직급코드가 J4와 J7인 직원들의 사원명, 급명, 급여를 조회하시오. (단, JOIN, IN 사용할 것)
