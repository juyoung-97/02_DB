...
-- VIEW 생성 권한 부여 (SYS -> kh_jjy 계정으로)
GRANT CREATE VIEW TO kh_jjy;


-- VIEW 생성 (kh_jjy 계정으로 접속 후 진행)
CREATE VIEW V_EMP
...
-- V_DCOPY2 VIEW 에 INSERT 수행
INSERT INTO V_DCOPY2
VALUES('D0', 'L2');
--> 가상 테이블 VIEW 에 INSERT 가 성공한것을 확인함
...
-- VIEW 생성 시 사용한 원본테이블 DEPT_COPY2에 
-- 값이 INSERT 됨을 확인

--> 모든 컬럼값이 INSERT 된것이 아니라
-- VIEW 를 생성할 때 사용된 컬럼에만 데이터가 사용되었고
-- 반대로, 사용되지 않은컬럼(DEPT_TITLE)에는 NULL 이 들어감
--> NULL 은 DB의 무결성을 약하게 만드는 주요 원인
-- 가능하면 의도되지 않은 NULL 은 존재하지 않게 해야함.

-- 무결성 : 데이터베이스에서 데이터를 정확하고 일관되게 유지하기 위한 중요한 개념.
-- 데이터의 정확성, 일관성, 신뢰성을 보장함.

-- WITH READ ONLY 옵션 사용하기
-- VIEW 를 이용해서 DML(INSERT / UPDATE / DELETE) 막기 위해 사용
...
INCREMENT BY 5 -- NEXTVAL 호출 시에 5씩 증가
...
--> CURRVAL 의 정확한 의미는 가장 최근 호출된 NEXTVAL 값을 반환함을 뜻함
 ...
 * 즉, 인덱스의 핵심 역할은
 * 컬럼값 -> 해당 행의 ROWID 를 통히 빠르게 찾아내는 것!
 * 
 * */
...
SELECT COUNT(*) FROM TB_IDX_TEST; -- 100만개 행 삽입
