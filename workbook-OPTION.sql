...
-- 2. 휴갛중인 학생들의 이름과 주민번호를 나이가 적은 순서로 화면에 출력하시오
...
-- (법학과의 '학과코드'는 학과 테이블(TB_DEPARTMENT)을 조회하여 찾아내도록 하자)

SELECT * FROM TB_PROFESSOR tp ;
SELECT * FROM TB_DEPARTMENT ; -- 법학과 학과코드 005
...
-- 9. 8의 결과 중 '인문사회' 계열에 속한 과목의 교수 이름을 찾으려고 한다
-- 이에 해당하는 과목 이름과 교수 이름을 출력하는 SQL 문 작성
...
-- 19. 춘 기술대학교의 "환경조경학과"가 속한 같은 계열 학과들의 학과 별 전공과목 평점을 파악하기 위한 적절한 SQL문 찾기
-- 단, 출력헤더는 "계열 학과명", "전공평점"으로 표시되도록하고, 평점은 소수점 한자리까지만 반올림하여 표시되도록 한다
...
WHERE MAIN.CATEGORY = (SELECT CATEGORY
					FROM TB_DEPARTMENT SUB
					WHERE SUB.DEPARTMENT_NAME = '환경조경학과')
