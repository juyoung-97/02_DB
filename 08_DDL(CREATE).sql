...
INSERT INTO MEMBER 
VALUES('MEM02', 'QWER1234', '김영희', '971229-1011211', SYSDATE);
...
-- 0으로 시작할 가능성이 있으면 CHAR, VARCHAR2 같은 문자형 이용해야함
...
-- ORA-01400: NULL을 ("KH_JJY"."USER_USED_NN"."USER_NO") 안에 삽입할 수 없습니다
...
-- 테이블에서 한 행의 정보를 찾기 위해 사용한 컬럼을 의미함
...
INSERT INTO USER_USED_PK VALUES
(1, 'USER02', 'PASS02', '이순신', '남자', '010-5383-8825', 'boduni35@kr.or.kr');
-- ORA-00001: 무결성 제약 조건(KH_JJY.USER_NO_PK)에 위배됩니다
--> 기본키 중복 오류!

INSERT INTO USER_USED_PK VALUES
(NULL, 'USER02', 'PASS02', '이순신', '남자', '010-5383-8825', 'boduni35@kr.or.kr');
-- ORA-01400: NULL을 ("KH_JJY"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다
--> 기본키 NULL 이므로 오류!

INSERT INTO USER_USED_PK VALUES
(2, 'USER02', 'PASS02', '이순신', '남자', '010-5383-8825', 'boduni35@kr.or.kr');
...
INSERT INTO USER_USED_PK2 VALUES
(2, 'USER02', 'PASS01', '홍길동', '남자', '010-1234-5678', 'hong@kr.or.kr');
-- ORA-00001: 무결성 제약 조건(KH_JJY.PK_USERNO_USERID)에 위배됩니다
--> USER_NO, USER_ID 둘 다 중복되었을 때만 제약조건 위배

INSERT INTO USER_USED_PK2 VALUES
(NULL, 'USER02', 'PASS01', '홍길동', '남자', '010-1234-5678', 'hong@kr.or.kr');
-- ORA-01400: NULL을 ("KH_JJY"."USER_USED_PK2"."USER_NO") 안에 삽입할 수 없습니다
--> 둘 중 하나라도 NULL 값이라면 제약조건 위배
...
-- 컬럼레벨일 경우
-- 컬럼명 자료형(크기) [CONSTRAINT 이름] REFERENCES 참조할 테이블명 [(참조할컬럼명)] [삭제룰]

-- 테이블레벨일 경우
-- [CONSTRAINT 이름] FOREIGN KEY (적용할컬럼명)
-- REFERENCES 참조할 테이블명 [(참조할컬럼명)] [삭제룰]

-- * 참조될 수 있는 컬럼은 PRIMARY KEY 컬럼과, UNIQUE 지정된 컬럼만 외래키로 사용 가능 *


-- 참조할 테이블의 참조할 컬럼명이 생략되면, PRIMARY KEY 로 설정된 컬럼이
-- 자동 참조할 컬럼이 됨
...
CREATE TABLE USER_USED_FK(
	...
	PHONE VARCHAR2(20),
	EMAIL VARCHAR2(50), 
	GRADE_CODE NUMBER CONSTRAINT GRADE_CODE_FK REFERENCES USER_GRADE /*(GRADE_CODE)*/ -- 컬럼레벨 작성법
...
-- USER_GRADE(부모테이블/참조테이블) 에 10 이라는 GRADE_CODE 가 존재하므로
-- 자식테이블이 10 이라는 값 사용 가능.

INSERT INTO USER_USED_FK VALUES 
(2, 'USER02', 'PASS02', '이순신', '남자', '010-3434-5656', 'lee@kr.or.kr', 10);
-- USER_GRADE(부모테이블/참조테이블) 에 10 이라는 GRADE_CODE 가 존재하므로
-- 자식테이블이 10 이라는 값 사용 가능.

INSERT INTO USER_USED_FK VALUES 
(3, 'USER03', 'PASS03', '유관순', '여자', '010-6969-7474', 'you@kr.or.kr', 30);
-- USER_GRADE(부모테이블/참조테이블) 에 10 이라는 GRADE_CODE 가 존재하므로
-- 자식테이블이 30 이라는 값 사용 가능.

INSERT INTO USER_USED_FK VALUES 
(4, 'USER04', 'PASS04', '전주영', '남자', '010-2186-8825', 'boduni20@kr.or.kr', NULL);
--> NULL 사용 가능

INSERT INTO USER_USED_FK VALUES 
(5, 'USER05', 'PASS05', '전교진', '남자', '010-5242-7472', 'solbi35@kr.or.kr', 50);
...
SELECT * FROM USER_USED_FK2; -- 10 을 사용하고 있던 자식 레코드값이 NULL 이 됨을 확인
...
INSERT INTO USER_USED_CHECK VALUES 
(2, 'USER02', 'PASS02', '유관순', '여자', '010-1111-2222', 'lee@kr.or.kr');
-- ORA-02290: 체크 제약조건(KH_JJY.GENDER_CHECK)이 위배되었습니다

INSERT INTO USER_USED_CHECK VALUES 
(2, 'USER02', 'PASS02', '유관순', '여', '010-1111-2222', 'lee@kr.or.kr');
-- 가능
